...
public class CollisionAvoidance : MonoBehaviour {
	...
	public Vector2 getSteering(ICollection<Rigidbody2D> targets)
	{
		...
		foreach (Rigidbody2D r in targets)
		{
			...
			Vector2 relativePos = (Vector2) transform.position - (Vector2) r.position;
			Vector2 relativeVel = (Vector2) rb.velocity - (Vector2) r.velocity;
		...
		{
			acceleration = (Vector2) transform.position - (Vector2) firstTarget.position;
		}
